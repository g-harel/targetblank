...
resource "aws_route53_record" "cert_validation" {
  name    = "${element(aws_acm_certificate.ssl_cert.domain_validation_options, 0).resource_record_name}"
  type    = "${element(aws_acm_certificate.ssl_cert.domain_validation_options, 0).resource_record_type}"
  zone_id = "${aws_route53_zone.primary.id}"
  records = ["${element(aws_acm_certificate.ssl_cert.domain_validation_options, 0).resource_record_value}"]
  ttl     = 60
}
